...
extern "C" void call_Adafruit_PN532_begin() { nfc.begin(); }
extern "C" uint32_t call_Adafruit_PN532_getFirmwareVersion() { nfc.getFirmwareVersion(); }
extern "C" bool call_Adafruit_PN532_setPassiveActivationRetries(uint8_t maxRetries) 
										{ return nfc.setPassiveActivationRetries(maxRetries); }
extern "C" bool call_Adafruit_PN532_SAMConfig(void) { return nfc.SAMConfig(); } 
extern "C" bool call_Adafruit_PN532_readPassiveTargetID
...
extern "C" bool call_Adafruit_PN532_inListPassiveTarget() { return nfc.inListPassiveTarget(); }

extern "C" bool call_Adafruit_PN532_inDataExchange(
					uint8_t * send, uint8_t sendLength, uint8_t * response, uint8_t * responseLength) 
					{	return nfc.inDataExchange(send, sendLength, response, responseLength); }

extern "C" void call_Println(char * str) { Serial.println(str); }

extern "C" void call_PrintValue(uint32_t d) { Serial.print("\nValue : "); Serial.print(d, HEX); }
...
uint32_t Adafruit_PN532::getFirmwareVersion(void) {
  ...
  if (0 != strncmp((char *)pn532_packetbuffer, (char *)pn532response_firmwarevers, 6)) {
	...
	Serial.print("Found chip PN5"); Serial.println((response>>24) & 0xFF, HEX); 
	//Serial.println((response>>24) & 0xFF); 
	Serial.print("Firmware ver. "); Serial.print((response>>16) & 0xFF, DEC); 
	//Serial.print((response>>16) & 0xFF); 
	Serial.print('.'); Serial.println((response>>8) & 0xFF, DEC);  
	//Serial.println((response>>8) & 0xFF);  
...
bool Adafruit_PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid, uint8_t * uidLength, uint16_t timeout) {
  ...
  if (!sendCommandCheckAck(pn532_packetbuffer, 3, timeout))
  {
    ...
    #endif
	Serial.println("Card is not found!");
...
bool Adafruit_PN532::inDataExchange(uint8_t * send, uint8_t sendLength, uint8_t * response, uint8_t * responseLength) {
  ...
  for (i=0; i<sendLength; ++i) {
	...
	Serial.print(" 0x"); Serial.print(send[i], HEX);
  }
  ...
  if (!sendCommandCheckAck(pn532_packetbuffer,sendLength+2,1000)) {
    ...
    #endif
	Serial.println("Could not send APDU");
    return false;
  }
...
bool Adafruit_PN532::inListPassiveTarget() {
  ...
  PN532DEBUGPRINT.println("Tap the Card");
  
  if (!sendCommandCheckAck(pn532_packetbuffer,3,3000)) {
    #ifdef PN532DEBUG
